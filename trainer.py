...
class SpeechLLMLightning(pl.LightningModule):
    def __init__(self, 
        ...
        self.connector = get_connector(connector_name, audio_enc_dim, llm_dim, connector_k, finetune_connector)
        self.llm_tokenizer, self.llm_model = get_llm(llm_name, use_lora, lora_r, lora_alpha, finetune_llm)
    ...
    def validation_step(self, batch, batch_idx):
            ...
            if 'Transcript' in extracted_target:
                ...
                predicted_transcript = extracted_pred.get('Transcript', '')
                ...
                # Save predictions for logging
                if batch_idx < 5:  # Save first 5 examples
    ...
    def test_step(self, batch, batch_idx):
            ...
            if 'Transcript' in extracted_target:
                ...
                # Log WER metric
                self.log("test/wer", wer_metric, on_step=False, on_epoch=True, prog_bar=True, logger=True)
    ...
    # Save predictions to file
        if self.current_epoch % 5 == 0:  # Save every 5 epochs
            with open(f'predictions_epoch_{self.current_epoch}.txt', 'w') as f:
                f.write(f"Validation Results - Epoch {self.current_epoch}\n")
                f.write("="*50 + "\n\n")
                for i, pred in enumerate(self.val_predictions):
                    f.write(f"Example {i+1}:\n")
                    f.write(f"Target: {pred['target']}\n")
                    f.write(f"Predicted: {pred['predicted']}\n")  
                    f.write(f"WER: {pred['wer']:.4f}\n")
                    f.write("-"*50 + "\n")
