...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels_str = {'Transcript': transcript}
        
        return waveform, labels_str
...
class InstructionalAudioDataset(AudioDataset):
    def __init__(self, csv_file, mode='train'):
        ...
        self.instruction_phrases = [
            ...
            "Provide the transcript of this audio",
            "What is the content of this speech?",
            "Tell me what is being said in this recording",
        ]
    
    def __getitem__(self, idx):
        waveform, labels_str = super().__getitem__(idx)
        instruction_phrase = random.choice(self.instruction_phrases)

        # Simplify prompts for ASR task
        pre_speech_prompt = f"Instruction:\n{instruction_phrase}\n\nInput:\n<speech>"
        post_speech_prompt = "</speech>\n\nOutput:\n"
        output_prompt = f'{{"Transcript": "{labels_str["Transcript"]}"}}'
        complete_prompt = pre_speech_prompt + post_speech_prompt + output_prompt
...
if __name__ == "__main__":
    ...
    waveform, pre_speech_prompt, post_speech_prompt, output_prompt, complete_prompt = dataset[0]
    print("\nExample prompt:")
    print(complete_prompt)
    print("\nWaveform shape:", waveform.shape if waveform is not None else None)